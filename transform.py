...
def normalize_category_fuzzy(value, choices, threshold=80):
    """ 
    Normalizar las categorías usando rapidfuzz
    """
    if isinstance(value, str):
        # Obtener el mejor match y su score
        best_match = process.extractOne(value, choices)
        if best_match and best_match[1] >= threshold:  # Verificamos que best_match existe y el score es suficiente
            return best_match[0]  # Devolver el nombre de la mejor coincidencia
    return 'nr'  # Devolver 'nr' si no hay coincidencia adecuada
...
# Compilar la expresión regular para eliminar términos irrelevantes
...
def group_similar(company, all_companies, threshold=85):
    """
    Usa extract para obtener las mejores coincidencias y agrupar compañías.
    """
    # Obtener coincidencias con process.extract de rapidfuzz
